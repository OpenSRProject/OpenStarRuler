...
public:
	...
	bool init() {
		...
		glDebugMessageCallback([](GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const void *userParam) {    // ignore non-significant error/warning codes
			...
			std::cout << std::endl;
			//print("GL CALLBACK: %s id = 0x%x (%d), type = 0x%x, severity = 0x%x, message = %s\n",
			//      (type == GL_DEBUG_TYPE_ERROR ? "** GL ERROR **" : ""), id, id, type, severity, message);
		}, nullptr);
		//Initialize state
